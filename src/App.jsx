...
function App() {
      ...
      try {
          ...
          if (firebaseUser) {
            ...
            if (!import.meta.env.VITE_INITIAL_AUTH_TOKEN) {
              ...
              try {
                ...
                console.error("Error signing in with custom token with Firebase:", error);
                await signInAnonymously(auth);
        ...
        if (storedUser && storedJwt) {
          try {
            ...
            console.log("App.jsx: JWT found in localStorage (first 10 chars):", storedJwt.substring(0, 10) + "...");

            // Fetch the user again with populate to get the latest player data
            const userResponse = await fetch(`${API_BASE}api/users/me?populate[jugador][populate]=*`, {
              headers: {
            ...
            if (userResponse.ok) {
              setUser(userData);
              console.log("App.jsx: Populated user data fetched successfully:", userData);
            } else {
              console.error("App.jsx: Error fetching populated user data:", userResponse.status, userData.error?.message);
              localStorage.removeItem('jwt');
              localStorage.removeItem('user');
              setUser(null);
            }
          } catch (e) {
            console.error("App.jsx: Error parsing stored user or fetching populated data:", e);
            localStorage.removeItem('user'); // Clear corrupted data
            localStorage.removeItem('jwt');
            setUser(null);
          }
        } else {
          console.log("App.jsx: No user or JWT found in localStorage on initial load.");
        }
        ...
        console.error("App.jsx: Error initializing Firebase or authentication:", error);
        setIsAuthReady(true);
      }
    };
  ...
  const handleLoginSuccess = (loggedInUser) => {
    ...
    console.log("App.jsx: JWT after login (first 10 chars):", jwt ? jwt.substring(0, 10) + "..." : "No JWT found immediately after login");
  ...
  const MobileNavBar = ({ user }) => {
    ...
    const baseButtonClasses = "flex flex-col items-center justify-center p-3 mb-2 bg-black bg-opacity-50 hover:bg-opacity-70 rounded-full shadow-md text-white transition duration-300 text-xs w-20 h-20 flex-shrink-0";
...
const MobileNavBarAlt = ({ user }) => {
  ...
  const handleLogoDoubleClick = () => {
    ...
    console.log("MobileNavBar toggled:", !isAltNavBarActive ? "Alt" : "Default");
  };
      ...
      <div className="min-h-screen bg-gray-100 py-6 flex flex-col items-center justify-center sm:py-12 pb-20">
        <div className="relative py-3 w-full sm:max-w-xl md:max-w-2xl lg:max-w-4xl mx-auto">
          ...
          <div className="relative bg-white shadow-lg sm:rounded-3xl p-4 sm:p-8">
            ...
            <Routes> {/* Routes debe estar dentro de Router */}
              <Route
                path="/"
                element={
                  <>
